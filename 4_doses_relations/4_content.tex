...
An illustration of the differences between classical DVH and after swapping $x$ and $y$-axes is available in figure \ref{fig:dose_volume_difference}.
...
This observation suggests lower numerical error and is another incentive to have the volume as the $x$-axis.

Calculating the total variation distance is computationally efficient, requiring only $\mathcal{O}(n_s)$ operations for each structure, where $n_s$ is the number of voxels in the structure of interest $s$.
