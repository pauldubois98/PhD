...
There is the possibility to disable penalisation once a constraint is met (this is useful for the first two methods presented above).
In our implementation, we chose not to, so that when a constraint is on the edge of being met, the penalization does not turn on and off every other optimization iteration.

Once the set of penalised voxels are selected, the penalisation power $p$ must be determined, with common choices being $p=1$ or $p=2$.
