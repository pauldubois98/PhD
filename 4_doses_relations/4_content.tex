...
It measures the minimum distance a particle must cover by moving along two curves simultaneously.
In our case, these are the DVH curves of two radiation doses.
...
Let them respectively be consisting of line segments joining the series of points $\left\lbrace \mathcal{C}_A(i) = (d_i, v_i), \ 1 \leq i \leq n_A \right\rbrace $ and $\left\lbrace \mathcal{C}_B(j) = (\tilde{d}_j, \tilde{v}_j), \ 1 \leq j \leq n_B \right\rbrace $; where $d_i$ and $\tilde{d}_j$ denote the dose levels, $v_i$ and $\tilde{v}_j$ represent the corresponding volumes, and $n_A$ and $n_B$ are the number of points forming $\mathcal{C}_A$ and $\mathcal{C}_B$\footnote{Since we are constantly comparing two DVH curves of the same structure, we always have $n_A = n_B$.}.
...
After testing on a few curves, this algorithm made calculations 3-5 times faster while affecting the value of the Fréchet distance by less than $0.5\%$.
It is, therefore, used in the results presented after that.
