...
For each pair of doses, we calculated the pairwise distances, effectively constructing the adjacency matrix of a fully connected graph, where each optimized dose corresponds to a node.
...
\begin{itemize}
	...
	\item It should remain small in cases where the voxel-wise distance is large, but the clinical significance of the two doses is similar, even if the doses are voxel-wise different.
\end{itemize}
