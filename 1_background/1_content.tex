...
Then, the MLC leaves are positioned so that the open area of the irradiation head matches the level set (figures \ref{fig:level_set_matching_with_leaves}, \ref{fig:leaves_angle_ter}).
Note that convex level sets can all be matched with the MLC leaves; if the level set is concave, changing the collimator angle may allow the leaves to match the level set shape (figure \ref{fig:leaves_angle}).
...
\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{_leaves_angle_ter.pdf}
...
The fluence is segmented in a one-dimensional fluence curve along each leaf pair axis.
...
The greater the time difference, the more rays will be sent from that point.
...
Solving a linear programming problem allows a leaf pair to deliver any fluence within arbitrary approximation in a reasonable amount of time.
