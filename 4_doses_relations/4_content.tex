...
The Hausdorff distance is another similarity metric between two curves \cite{Henrikson1999CompletenessAT}.
It is the maximum distance between any point on one curve and its closest point on the other.
...
Our analysis treats the DVH curves as sets of points in a 2D space ($\R^2$) and uses the Hausdorff distance to measure their difference.
Using the same notation for DVH curves $\mathcal{C}_A$ and $\mathcal{C}_B$ as before, the discrete Hausdorff distance can be computed as follows:
