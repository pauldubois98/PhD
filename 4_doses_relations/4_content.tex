...
We optimize each constraint with all weights set to $1$, but we sequentially set one to $3$, $10$, and $100$.
This process gives us 18 doses that we can compare.
We can calculate the pairwise distance between each pair of doses using the distance techniques described above.
Considering each optimized dose as a node, we can create a graph for each defined distance; calculating the pairwise distance is equivalent to calculating the adjacency matrix of the dense graph.
...
\begin{itemize}
	\item matches the voxel distance when the voxel distance is low
	\item remains small in some cases of a high voxel distance when the clinical meaning of the two doses is the same, despite being voxel-wise (very) different
\end{itemize}

Observing pairwise distances in Fig. \ref{fig:pairwise_distances}, we have that:
\begin{itemize}
	\item Frechet and Haussdorff are very similar to voxel-wise; we interpret them as too sensitive.\\
