...
In this method, a fully automated process generates a single optimal plan without the need for human interaction.
Breedveld et al. introduced this approach in their work on IMRT Cycle (iCycle), where the optimal plan is guided by a predefined "wish list" of dosimetric criteria, each assigned a specific priority.
...
However, Craft and Bortfeld analyzed head and neck IMRT plans and demonstrated that only a small number of plans are needed to form a Pareto database.
...
This insight facilitated the practical clinical use of MCO, first implemented in the RayStation TPS.
