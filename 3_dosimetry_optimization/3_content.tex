% dose is v sensible (+ ref)
% 1Gy difference may have different effect (79 to 80 GY us fine, but 80 to 81 may not be)
% organs have very different sensibilities to dose
% e.g.: some organs can have lots of Gy if well spread
% while some others can have a lot in one point (called a heatpoint), but can't handle much overall
% hence, on top of the prescription (e.g.: 80Gy on PTV), there are DVH constraints formulated by doctors
% the goal is ofc to have 0Gy on OARs, but this is v difficult
% here, we explore different techniques in order to find fluence maps that best fit the constraints
